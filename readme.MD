...
This project supports API key rotation, which can help to:
- Distribute load across multiple API keys
- Avoid rate limits by cycling through different keys
- Provide higher availability when some keys might be unavailable

### Setting up key rotation

There are several ways to configure and use API keys:

1. **Predefined Keys**: The project comes with a set of predefined API keys that are ready to use.
   - These keys are automatically used in rotation
   - Every minute, the system automatically switches to the next key in the rotation

2. **Environment Variable**: Set the `GEMINI_API_KEYS` environment variable with comma-separated API keys:
   ```
   GEMINI_API_KEYS=key1,key2,key3
   ```
   - These keys will be added to the rotation pool along with the predefined keys

3. **Admin API**: Use the admin API to dynamically manage keys at runtime:
   - Set the `ADMIN_KEY` environment variable first
   - Then use the `/admin/keys` endpoint with the following methods:
     - `GET` - List all keys (obfuscated)
     - `POST` - Add a new key with `{"key": "your-api-key"}`
     - `DELETE` - Remove a key with `{"key": "your-api-key"}`
   - All admin API requests must include the `X-Admin-Key` header with your admin key

4. **Special Client Usage**: Send `Bearer rotate` in the Authorization header to force using the next key in rotation

> [!NOTE]
> - Keys are automatically rotated every minute to distribute load
> - When no keys are available, the client must provide a valid API key in the Authorization header
> - When keys are configured, the client can either provide its own key or use `rotate` to use one from the server's rotation
